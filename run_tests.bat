@echo off
REM Run the test stack with Docker Compose and .env_test
call docker compose -f docker-compose.test.yml --env-file .env_test up --build --abort-on-container-exit
...
call docker compose -f docker-compose.test.yml --env-file .env_test down -v
